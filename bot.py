...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if starts_with:
        ...
        if (chat_id not in chat_emails or not chat_emails.get(chat_id)) and (original_chat_id not in chat_emails or not chat_emails.get(original_chat_id)):
    ...
    # Если сообщение начинается с ключевого слова без учета регистра
        ...
        logger.info(f"Проверка: '{message_text[:len(keyword) if len(keyword) <= len(message_text) else len(message_text)]}' == '{keyword}'")
